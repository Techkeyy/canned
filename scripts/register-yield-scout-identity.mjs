...
if (env.CANNED_ALLOW_TESTNET_WRITES !== "true" || env.CANNED_YIELD_REGISTER_CONFIRM !== "true") throw new Error("ERC-8004 registration requires explicit testnet write and registration confirmations; no write was attempted.");
const agentUrl = env.CANNED_YIELD_AGENT_URL;
if (!isPublicHttpUrl(agentUrl)) throw new Error("Registration requires a public Yield Scout URL; local URLs are rejected.");
...
const quote = await requestJson(at("/negotiate"), { method: "POST", headers: { "Content-Type": "application/json" }, body: { task_description: "YieldBench v1 registration readiness probe; no job will be created.", terms: { deliverables: "Signed readiness response only", quality_standards: "Must identify BSC Testnet, U, price, and expiry", success_criteria: ["No onchain job"] }, request_id: `yield-registration-${Date.now()}` } });
...
const walletsDir = path.join(dataDir, "state", REFERENCE_WALLET_PATHS.yield.walletsDir);
const passwordFile = path.join(dataDir, "state", REFERENCE_WALLET_PATHS.yield.passwordFile);
const password = (env.CANNED_YIELD_PROVIDER_PASSWORD || await readFile(passwordFile, "utf8")).trim();
const providerAddress = env.CANNED_YIELD_PROVIDER_ADDRESS || EVMWalletProvider.listWallets(walletsDir)[0];
if (!providerAddress) throw new Error("Yield provider wallet is not configured.");
if (String(providerAddress).toLowerCase() !== String(status.body.provider).toLowerCase()) throw new Error("The local Yield provider wallet does not match the live provider address.");

// Two reference agents must never resolve to the same identity or endpoint.
const siblings = {};
for (const [key, file] of [["health-factor", "reference-health-identity.json"], ["rebalancing", "reference-range-identity.json"]]) {
...
for (const [key, record] of Object.entries(siblings)) {
  if (!record) continue;
  if (String(record.provider).toLowerCase() === String(providerAddress).toLowerCase()) throw new Error(`Yield Scout must not reuse the ${key} provider wallet.`);
  if (record.endpoint === agentUrl) throw new Error(`Yield Scout must not reuse the ${key} endpoint.`);
}
...
try {
  ...
  const result = await sdk.registerAgent(agentUri, [
    ...
    { key: "venue", value: metadata.venue || "Venus" },
  ...
  const fleetFailures = referenceFleetIdentityFailures({ ...siblings, yield: record });
  ...
  await writeFile(path.join(dataDir, "state", "reference-yield-identity.json"), `${JSON.stringify(record, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
