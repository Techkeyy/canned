...
import { rebalanceBenchProviderTask, REBALANCE_BENCHMARK_ID } from "../src/reference/rebalance-benchmark.mjs";
...
if (!publicConfig.valid) throw new Error(`Public Range Keeper configuration rejected: ${publicConfig.errors.join(", ")}`);
const fulfillmentEnabled = env.CANNED_RANGE_ENABLE_FULFILLMENT === "true";
if (fulfillmentEnabled && env.CANNED_RANGE_ALLOW_TESTNET_WRITES !== "true") throw new Error("Fulfillment requires CANNED_RANGE_ALLOW_TESTNET_WRITES=true; no write was attempted.");
...
const providerAddress = env.CANNED_RANGE_PROVIDER_ADDRESS || (configuredAddresses.length === 1 ? configuredAddresses[0] : null);
if (!providerAddress) throw new Error("CANNED_RANGE_PROVIDER_ADDRESS is required.");
...
const runtime = new ReferenceAgentRuntime({ spec, taskHandler: ({ jobId, task }) => buildRangeKeeperDeliverable({ jobId, task }) });
if (fulfillmentEnabled) runtime.heartbeat({ state: "idle" });
const storageDir = path.join(dataDir, "state", "range-deliverables");
...
const registeredAgentId = env.CANNED_RANGE_AGENT_ID ? Number(env.CANNED_RANGE_AGENT_ID) : null;
const registeredRegistry = env.CANNED_RANGE_REGISTRY || null;
if (registeredAgentId !== null && (!Number.isInteger(registeredAgentId) || !registeredRegistry)) throw new Error("CANNED_RANGE_AGENT_ID requires a matching CANNED_RANGE_REGISTRY.");
...
const taskFile = env.CANNED_RANGE_TASK_FILE ? path.resolve(env.CANNED_RANGE_TASK_FILE) : null;
...
async function taskFromFile() {
  if (!taskFile) throw new Error("RebalanceBench task file is not configured; no task was executed.");
  const definition = JSON.parse(await readFile(taskFile, "utf8"));
  if (definition?.benchmarkId !== REBALANCE_BENCHMARK_ID || definition?.immutable !== true) throw new Error("Only an immutable RebalanceBench_v1 definition may be executed.");
  return rebalanceBenchProviderTask(definition);
}
...
const bindHost = env.CANNED_RANGE_BIND_HOST || "0.0.0.0";
const port = Number(env.PORT || env.CANNED_RANGE_PORT || 8791);
