...
if (!publicConfig.valid) throw new Error(`Public Grid Keeper configuration rejected: ${publicConfig.errors.join(", ")}`);
const fulfillmentEnabled = env.CANNED_GRID_ENABLE_FULFILLMENT === "true";
if (fulfillmentEnabled && env.CANNED_GRID_ALLOW_TESTNET_WRITES !== "true") throw new Error("Fulfillment requires CANNED_GRID_ALLOW_TESTNET_WRITES=true; no write was attempted.");
...
const providerAddress = env.CANNED_GRID_PROVIDER_ADDRESS || (configuredAddresses.length === 1 ? configuredAddresses[0] : null);
if (!providerAddress) throw new Error("CANNED_GRID_PROVIDER_ADDRESS is required.");
...
const registeredAgentId = env.CANNED_GRID_AGENT_ID ? Number(env.CANNED_GRID_AGENT_ID) : null;
const registeredRegistry = env.CANNED_GRID_REGISTRY || null;
if (registeredAgentId !== null && (!Number.isInteger(registeredAgentId) || !registeredRegistry)) throw new Error("CANNED_GRID_AGENT_ID requires a matching CANNED_GRID_REGISTRY.");
...
/**
 * GridBench is frozen in code and content addressed, so the provider does not
 * read a task file for it. A funded job is answered from the frozen
 * definition, and the precommit is checked before anything is served.
 */
async function gridBenchTask() {
  const definition = buildGridBenchmarkDefinition();
  if (definition.benchmarkId !== GRID_BENCHMARK_ID) throw new Error("Only the frozen GridBench v1 definition may be executed.");
  return { benchmarkId: definition.benchmarkId, precommit: definition.precommit, packet: publicGridBenchPacket(definition) };
}
...
const bindHost = env.CANNED_GRID_BIND_HOST || "0.0.0.0";
const port = Number(env.PORT || env.CANNED_GRID_PORT || NAMESPACE.port);
...
if (fulfillmentEnabled) {
  await startReferenceWatcher({ seller, runtime, interval: 15, stop: abort.signal, taskResolver: async () => gridBenchTask() });
} else {
