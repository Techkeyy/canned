...
const agentUrl = process.env.CANNED_RANGE_AGENT_URL;
if (!isPublicHttpUrl(agentUrl)) throw new Error("CANNED_RANGE_AGENT_URL must be a public HTTP(S) URL; localhost is not accepted.");
...
const quoteRequest = {
  task_description: "RebalanceBench v1 readiness probe; no job will be created.",
  ...
  request_id: `range-readiness-${Date.now()}`,
};
...
if (quote.ok && envelope.provider_sig && envelope.negotiation_hash) {
  const rpcUrl = process.env.RPC_URL_BSC_TESTNET || process.env.CANNED_RPC_URL || "https://bsc-testnet-rpc.publicnode.com";
  ...
  } catch { failures.push("quote_signature_read"); }
...
// A Range Keeper that could move capital would invalidate the benchmark.
...
if (failures.length) throw new Error(`Range Keeper readiness failed: ${[...new Set(failures)].join(", ")}`);
console.log(JSON.stringify({
  status: "range_keeper_readiness_verified",
  agent: spec.name,
