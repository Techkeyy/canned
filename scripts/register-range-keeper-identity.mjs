...
import { referenceSpec, REFERENCE_CHAIN_ID, REFERENCE_ERC8183_COMMERCE_PROXY, REFERENCE_NETWORK, REFERENCE_PAYMENT_TOKEN } from "../src/reference/constants.mjs";
...
const spec = referenceSpec("rebalancing");
const expectedCategory = CATEGORY_LABELS[spec.category];
if (env.CANNED_ALLOW_TESTNET_WRITES !== "true" || env.CANNED_RANGE_REGISTER_CONFIRM !== "true") throw new Error("ERC-8004 registration requires explicit testnet write and registration confirmations; no write was attempted.");
const agentUrl = env.CANNED_RANGE_AGENT_URL;
if (!isPublicHttpUrl(agentUrl)) throw new Error("Registration requires a public Range Keeper URL; local URLs are rejected.");
...
const quote = await requestJson(at("/negotiate"), { method: "POST", headers: { "Content-Type": "application/json" }, body: { task_description: "RebalanceBench v1 registration readiness probe; no job will be created.", terms: { deliverables: "Signed readiness response only", quality_standards: "Must identify BSC Testnet, U, price, and expiry", success_criteria: ["No onchain job"] }, request_id: `range-registration-${Date.now()}` } });
...
const password = (env.CANNED_RANGE_PROVIDER_PASSWORD || await readFile(passwordFile, "utf8")).trim();
const providerAddress = env.CANNED_RANGE_PROVIDER_ADDRESS || EVMWalletProvider.listWallets(walletsDir)[0];
if (!providerAddress) throw new Error("Range provider wallet is not configured.");
if (String(providerAddress).toLowerCase() !== String(status.body.provider).toLowerCase()) throw new Error("The local Range provider wallet does not match the live provider address.");

// Two reference agents must never resolve to the same identity or endpoint.
const healthRecord = await readFile(path.join(dataDir, "state", "reference-health-identity.json"), "utf8").then(JSON.parse).catch(() => null);
if (healthRecord && String(healthRecord.provider).toLowerCase() === String(providerAddress).toLowerCase()) throw new Error("Range Keeper must not reuse the Health Guard provider wallet.");
if (healthRecord && healthRecord.endpoint === agentUrl) throw new Error("Range Keeper must not reuse the Health Guard endpoint.");
...
try {
  ...
  const result = await sdk.registerAgent(agentUri, [
    ...
    { key: "execution_policy", value: "recommendation_only_no_capital_movement" },
  ]);
  ...
  const fleetFailures = referenceFleetIdentityFailures({ "health-factor": healthRecord, rebalancing: record });
  ...
  await writeFile(path.join(dataDir, "state", "reference-range-identity.json"), `${JSON.stringify(record, null, 2)}\n`, { encoding: "utf8", mode: 0o600 });
  console.log(JSON.stringify({ status: "erc8004_registered", agent: metadata.name, network: REFERENCE_NETWORK, chainId: REFERENCE_CHAIN_ID, agentId: result.agentId, registry: sdk.contractAddress, transactionHash: result.transactionHash, endpoint: agentUrl, provider: wallet.address, distinctFromHealthGuard: healthRecord ? healthRecord.agentId !== result.agentId : true, indexer: "pending_independent_8004scan_check", secretOutput: "none" }, null, 2));
