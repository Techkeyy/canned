...
The primary judge-facing marketplace is available at https://canned-lilac.vercel.app. Vercel serves the static frontend and exact read-only API rewrites; the live Node engine and summary API remain on the VPS at https://canned.103-195-188-198.sslip.io as the fallback/engine reference. The exact qualifying TermiX task, agent, and control payloads are publicly available under [evidence/termix](https://github.com/Techkeyy/canned/tree/master/evidence/termix) in the release repository.
