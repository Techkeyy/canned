...
test("Vercel exposes only the exact safe commerce proxy routes", () => {
  ...
  for (const source of ["/api/hire/quote", "/api/hire/prepare", "/api/hire/submit", "/api/hire/mine", "/api/hire/job/:hireId", "/api/hire/job/:hireId/result", "/api/hire/job/:hireId/evidence", "/api/claim/challenge", "/api/claim/verify", "/api/list/submit", "/api/grid/leash/proposal"]) assert.ok(sources.includes(source), `${source} must be explicitly proxied`);
