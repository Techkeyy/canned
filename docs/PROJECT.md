...
The primary judge-facing deployment is https://canned-lilac.vercel.app. Vercel serves the static frontend and exact read-only API rewrites; https://canned.103-195-188-198.sslip.io remains the live engine and fallback summary deployment. Its transfer remains summary-evidence-only; the three required raw TermiX task/agent/control payload sets are publicly available under [`evidence/termix`](https://github.com/Techkeyy/canned/tree/master/evidence/termix). Mutable runtime state remains outside both public surfaces.
